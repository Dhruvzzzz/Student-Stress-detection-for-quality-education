...
def real_time_camera_detection():
    ...
    try:
        ...
        with tab2:
            ...
            if st.button("Start 10-Second Recording", key="start_recording_btn"):
                ...
                if stress_history:
                    ...
                    df = pd.DataFrame(stress_history, columns=['Time (s)', 'Stress Level (%)'])
                    csv_file = f'stress_history_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        ...
        # Add high stress monitoring section
        st.markdown("<div class='subheader'><h3>High Stress Monitoring</h3></div>", unsafe_allow_html=True)
        st.markdown("""
        <div class='feature-card'>
            <h3>Extended Stress Monitoring</h3>
            <p>This feature monitors stress levels over three 30-second periods. If consistently high stress is detected, 
            an alert will be sent to the designated contact.</p>
        </div>
        """, unsafe_allow_html=True)

        col1, col2 = st.columns(2)
        with col1:
            student_name = st.text_input("Student Name", key="high_stress_name")
        with col2:
            recipient_email = st.text_input("Alert Email Address", key="alert_email")

        if st.button("Start High Stress Monitoring", key="high_stress_btn"):
            if not student_name or not recipient_email:
                st.error("Please provide both student name and email address")
            else:
                with st.spinner("Monitoring stress levels..."):
                    stress_data = monitor_high_stress(face_cascade, model, student_name, recipient_email)
                    
                    if stress_data:
                        # Display monitoring results
                        st.markdown("<div class='subheader'><h3>Monitoring Results</h3></div>", unsafe_allow_html=True)
                        stress_plot = create_stress_plot([(t, s) for t, s in stress_data])
                        if stress_plot:
                            st.pyplot(stress_plot)
...
def send_stress_alert_email(student_name, stress_data, recipient_email):
    """Send email alert for consistently high stress levels"""
    try:
        # Email configuration
        sender_email = "luvkushsunny@gmail.com"  # Your Gmail address
        sender_password = "vqtt aujs ooun zcbb"  # Your Gmail App Password
        
        msg = MIMEMultipart()
        msg['From'] = sender_email
        msg['To'] = recipient_email
        msg['Subject'] = f"High Stress Alert - {student_name}"
        
        body = f"""
        High Stress Alert Report
        ...
        for time_stamp, stress_level in stress_data:
            body += f"\nTime: {time_stamp} - Stress Level: {stress_level:.1f}%"
        
        body += f"""
        {'-' * 50}
        
        Recommendations:
        1. Schedule an immediate counseling session
        2. Review current workload and deadlines
        3. Consider stress management workshops
        4. Implement regular breaks and relaxation techniques
        
        This is an automated alert from the Student Stress Detection System.
        """
        ...
        st.info("Please check your email configuration and internet connection")
        return False

def monitor_high_stress(face_cascade, model, student_name, recipient_email):
    """Monitor for persistent high stress and trigger alerts"""
    high_stress_count = 0
    stress_data = []
    monitoring_rounds = 3
    buffer_time = 5  # Changed to 5 seconds
    
    st.warning("Beginning high stress monitoring...")
    
    for round in range(monitoring_rounds):
        st.info(f"Monitoring Round {round + 1}/{monitoring_rounds}")
        ...
        start_time = datetime.now()
        end_time = start_time + timedelta(seconds=buffer_time)
        round_stress_readings = []
        ...
        try:
            while datetime.now() < end_time:
                ...
                # Show progress
                elapsed = (datetime.now() - start_time).total_seconds()
                progress = min(100, (elapsed / buffer_time) * 100)
                progress_placeholder.progress(int(progress))
                ...
                for (x, y, w, h) in faces:
                    ...
                    stress_level = model.predict(face_resized, verbose=0)[0][0] * 100
                    round_stress_readings.append(stress_level)
                    
                    # Visual feedback - changed threshold to 50%
                    color = (0, 0, 255) if stress_level > 50 else (0, 255, 0)
                    cv2.rectangle(frame, (x, y), (x+w, y+h), color, 2)
                    cv2.putText(frame, f"Stress: {stress_level:.1f}%", 
                ...
                <div class='stress-monitor'>
                    <h4>Monitoring Round {round + 1}/{monitoring_rounds}</h4>
                    <p>Time Remaining: {buffer_time - elapsed:.1f}s</p>
                </div>
                """, unsafe_allow_html=True)
        ...
        if round_stress_readings:
            avg_stress = sum(round_stress_readings) / len(round_stress_readings)
            stress_data.append((datetime.now().strftime('%Y-%m-%d %H:%M:%S'), avg_stress))
            
            # Changed threshold to 50%
            if avg_stress > 50:
                high_stress_count += 1
                st.warning(f"High stress detected in round {round + 1} - Average: {avg_stress:.1f}%")
            else:
                st.success(f"Normal stress levels in round {round + 1} - Average: {avg_stress:.1f}%")
        
        if round < monitoring_rounds - 1:
            with st.spinner("Short break between rounds..."):
                time.sleep(2)  # Reduced break time between rounds
    
    # Final analysis and email alert
    if high_stress_count >= 2:  # Changed to trigger after 2 rounds of high stress
        st.error("""
        ⚠️ Persistent High Stress Detected!
        Sending alert email to designated contact...
        """)
        
        if send_stress_alert_email(student_name, stress_data, recipient_email):
            st.success("Alert email sent successfully!")
        else:
            st.error("Failed to send alert email. Please check the email configuration.")
    else:
